...
- Phase 7 must reconcile the gap ledger against current proof. If a real fix is proven but its gap row remains Open, close that row in Phase 7 without replaying implementation gates. If the fix/proof is actually missing, repair it in its owning phase before completion.
...
Follow the main Evidence And Resume Contract throughout Phases 5–7. Load this grouped reference once; reuse it across transitions with intact context. Keep browser/test/timeout/fixed-wait results only in their owning artifact. Batch checkpoint updates at meaningful work boundaries, blockers, and transitions. Later phases cite current earlier proof instead of duplicating command output, screenshot inventories, and test-removal/coverage recaps.

For record-only contradictions with valid proof, update the authoritative record once in the current phase. A real failed or missing check must be repaired in its owning phase; refresh only affected evidence and reuse unaffected gates without phase-by-phase paperwork.
...
14. A broken flow, relevant bad-case regression, missing screenshot, or responsive issue fails Phase 5. Record the issue once in open-gaps.md with its evidence pointer. For app-code fixes, repair in the owning implementation phase and refresh affected static/unit/browser claims only. A verification-script/selector or screenshot-path issue is repaired in Phase 5; it does not reopen implementation gates. Reuse unchanged coverage decisions and results.
...
18. At meaningful batch boundaries/promotion, checkpoint Phase 5’s artifact pointer and next action. Keep coverage, images, lifecycle and review details only in their owning Phase 5 sections.
...
22. Reuse this loaded reference for Phase 6 unless context was lost.
...
18. At meaningful batch boundaries/promotion, checkpoint Phase 6’s artifact pointer and next action. Keep rationale/results once in its Coverage Decisions, New E2E Burden Ledger, and Command Results sections.
19. Replace all `open-gaps.md` placeholder rows with real rows or explicit `None currently recorded` rows.
20. Cover every required semantic category once using the compact Phase 6 template: coverage/actions/pruning/removal rationale, per-new-case burden proof, functional assertions, bounded command scope/output/retry/triage, lifecycle/DB safety, fixed-wait review, and actual gaps. If no E2E is warranted, record a defended N/A rather than filling multiple empty audit tables.
21. After the Phase 6 gate passes, set `task-workflow/CURRENT_PHASE.txt` to `phase-7-final-signoff`.
22. Reuse this loaded reference for Phase 7 unless context was lost.
...
- Phase 7 found substantive missing/failed/invalidated work but did not repair it in its owning phase before completion
...
If this gate fails, reconcile record-only issues in Phase 7 or repair substantive failures in their owning phase under scoped invalidation.
