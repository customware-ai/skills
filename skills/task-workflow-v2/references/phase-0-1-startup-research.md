...
Load this reference once for Phases 0–1; reuse it across their transition while context is intact. Follow the main Evidence And Resume Contract for checkpointing, readbacks, and invalidation.
...
8. Fill the compact checkpoint with task/repo, essential constraints, current phase/artifact/reference, and next action. Seed its Phase 1 input-manifest pointer; do not create an additional Refs inventory or phase index.
9. After recording the Phase 0 passing score/critical result/decision from scaffold evidence, set `task-workflow/CURRENT_PHASE.txt` to `phase-1-task-research`.
10. Reuse this loaded reference for Phase 1; no reference reread is required while context is intact.
11. Update `task-workflow/progress.md` so current phase, current phase reference, and next local action match Phase 1.
12. Use the passing Phase 0 decision already established before promotion. Do not reopen its artifact merely to repeat marker or promotion proof.
...
- Phase 0 passes before promotion; the next marker/action is `phase-1-task-research`
...
21. Write the compact checkpoint with task/essential constraints, current phase/artifact/reference, active work and next action. All detailed research, selected inputs, rules, and plan live once in Phase 1; point there rather than copying them.
22. Set the checkpoint input-manifest pointer to Phase 1’s Inputs Read table for resume reloads of binding instructions and relevant inputs.
23. After every Phase 1 gate requirement passes, set `task-workflow/CURRENT_PHASE.txt` to `phase-2-execution`.
24. Load `references/phase-2-4-execution-integrity.md` before Phase 2 work if it is not already in context.
...
Research must let another agent resume using the artifact plus original inputs and code. Record decisions, relevant rules, affected areas, and evidence pointers; do not transcribe source declarations or facts that can be read directly from those files.
...
- `task-workflow/progress.md` not updated with enough context and a current phase/artifact/reference and input-manifest pointer to resume Phase 2 after compaction
...
- selected relevant skills are missing from Phase 1’s input manifest or its checkpoint pointer
...
- the checkpoint points to Phase 1’s input manifest, which records binding instructions, task/domain, all attachments, and selected relevant skills for resume
...
Use the already-established Phase 0 and Phase 1 passing decisions, scores, critical results, and concrete evidence. Record Phase 1’s pass, set the Phase 2 marker, and checkpoint the next action together. Load Phase 2’s reference if not already loaded. Do not re-open prior artifacts to authorize implementation while their results remain current.
