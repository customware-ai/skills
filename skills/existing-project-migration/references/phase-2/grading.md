...
5. Treat phase 2 as passed only when all critical items pass and at least `24/25` items pass.
...
12. Data-backed screens use seeded or imported data when available, or the original source screen chrome and control surface when data is sparse.
...
19. `CRITICAL` Playwright end-to-end coverage exists or was repaired for the workflows and routes that were actually verified.
20. `CRITICAL` `migration-review.md` records score, failed items, pass/fail call, verified workflows, commands run, and one row per verified source-visible screen or documented blocker.
21. `CRITICAL` `migration-open-gaps.md` was updated after verification passes and no ordinary unresolved gap, including user-visible drift, remains open.
22. The first real verify pass did not falsely declare zero gaps without adversarial review.
23. The adversarial final pass evaluated at least five suspected route or screen drifts and either fixed them or justified them specifically.
