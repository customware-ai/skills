...
- Phase 3 reviews connected places, closes missed implementation gaps, then owns the ordered static/build checkpoint: typecheck, lint, build. When one of those commands fails, fix all issues available from that command output as a batch, rerun that command, and only then continue.
