...
- In Phase 5 and Phase 6, use `task-workflow/scripts/playwright-lifecycle.mjs` for custom Playwright scripts or browser probes that need app startup, readiness, browser preflight, output capture, and cleanup. If the repo's Playwright config has a `webServer`, run native `pnpm exec playwright test ...` through that repo-owned lifecycle unless the config is explicitly disabled or made reuse-safe for a helper-owned server.
- Do not compose manual server cleanup, fixed sleep, DB-delete, server-start, and Playwright command chains in Phase 5 or Phase 6. Put pre-server setup such as DB reset, migration, or seed into lifecycle `--setup "..."` for helper-owned runs, or use the repo's Playwright `webServer` lifecycle for native Playwright tests. If the server appears stale or wrong, probe it and restart through the owning lifecycle path; broad process cleanup is only a recorded sandbox recovery after PID/port cleanup is impossible.
...
- Select test commands by the smallest useful scope. Start with targeted tests for changed logic, components, routes, or user flows; then run connected or affected tests. Phase 3 is a code/artifact second-pass review, not a broad regression-test phase; do not use full unit/Vitest or full Playwright/E2E there to "make sure nothing broke" or "understand current state." For E2E, keep an affected-spec ledger; if targeted or multi-spec commands already cover every directly affected spec, do not rerun the same coverage as an unfiltered full suite. Do not run broad/full unit, Vitest, or Playwright suites as default final confidence checks. Use them only when target repo instructions explicitly require them, the task explicitly asks for them, global/shared infrastructure changed, or earlier output is incomplete/stale; record that concrete reason before running the command.
...
- running Phase 5 or Phase 6 Playwright/E2E commands without the correct lifecycle owner recorded: helper for custom scripts/browser probes, repo Playwright `webServer` for native Playwright tests when configured
...
- running the unfiltered full Playwright/E2E suite when the task did not explicitly ask for it, the target repo did not require it, and the affected-spec ledger does not prove every E2E spec is directly affected
