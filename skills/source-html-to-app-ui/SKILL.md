...
9. Without reading the design JSON or target files, run `node task-workflow/scripts/validate-source-discovery.mjs task-workflow/source-playwright/inventory-source-discovery.mjs`. An absolute path is acceptable only when it resolves to these exact supplied scripts; identity and bootstrap hash must still match. Run every command printed by the validator separately and sequentially. The validated runner automatically shards large inventories into bounded lifecycle-owned execution batches, but the supplied script must still capture every finalized page at desktop and mobile, every visible section, every state at desktop and mobile, and desktop/mobile shell evidence after motion settles. Batch size is never an evidence limit; there is no page, state, section, or image limit. Read the completed manifest. Its dynamic `inspectionImages` contact sheets visibly cover every raw `images` evidence file exactly once, grouped only for readable inspection. Open every `inspectionImages` sheet in listed order—never a representative subset. A faded, blank, partially populated, moving, or transition-corrupted panel is invalid evidence and fails Phase 0. Only after every sheet has been inspected and found complete may you add `imagesOpened` as an exact ordered copy of `inspectionImages`.
10. Immediately after that one manifest edit, read `task-workflow/spec.json` directly and completely as the next tool action. Do not insert `ls`, `find`, glob, search, `stat`, a target file read, or repository probe before that design read. Only afterward may you inspect target implementation and existing target brand assets read-only. Before selecting packet owners, run `node task-workflow/scripts/inventory-phase-1-owners.mjs`, then read `task-workflow/phase-1-owner-candidates.json` in full. The inventory is the hard path authority even when target paths were also inspected manually. Packet 1 is strictly limited to the smallest needed candidates categorized `styles-and-tokens`, `shared-primitives`, or `ui-config`; route, page, shell, layout, and navigation owners/outcomes belong to later packets. Read every selected file separately before preparing the packet. If the expected brand directory is absent, record that fact without creating it. Then run `prepare-phase-1-packet.mjs` with exact first-packet contract IDs, those already-read candidate paths, and one UI-only tokens/theme/typography/primitives outcome. Every comma-separated `--evidence` value must be an existing captured evidence path beginning literally with `task-workflow/source/`; use the discovery manifest plus relevant inspection images. Never pass `task-workflow/spec.json`, `task-workflow/source-input/approved.html`, a target path, or another input as evidence. Its output and generated plan contain the literal Phase 0 closeout sequence. Execute every listed read and write separately, in order, including all three final readbacks, before invoking promotion. Do not score or invoke promotion until the handoff, every Phase 0 artifact row, `open-gaps.md`, `progress.md`, score, decision, promotion lock, and readback are complete. The promotion gate is a final audit, not a missing-item discovery loop. Calling it early is a failed run even when it rejects the attempt. Do not fetch, download, or open any externally referenced source brand asset in Phase 0; record its URL and intended Phase 1 destination only. Do not author or modify source capture code. Do not read another phase reference until its marker is set by the supplied promotion script.

The packet handoff is an atomic Phase 0 sequence. Complete every optional target-architecture and brand-asset inspection before running `inventory-phase-1-owners.mjs`; after that command starts, no target, public, brand, logo, directory, or repository probe is permitted. After `phase-1-owner-candidates.json` reaches EOF, choose the packet-1 owners. Read each selected owner once, separately, with no unselected implementation read between them. The literal next tool action after the last selected owner read must be `prepare-phase-1-packet.mjs` with those exact paths. Its successful result makes `phase-1-entry-plan.json` the literal next read. Until that plan reaches EOF, do not read or write `phase-0-source-contract.md`, `open-gaps.md`, `progress.md`, another implementation file, or another reference. Skipping packet preparation, reopening a Phase 0 ledger first, checking brand assets late, or adding another repository probe is automatic run failure and requires clean reset.
...
Every instruction to read a file "completely", "to EOF", or "back in full" is a hard multi-call read protocol, not a request for one tool call. After each read, inspect the tool result. If it reports `truncated: true`, a last line below `totalLines`, or another continuation offset, the literal next tool action must read the same file from that next unread offset. Repeat without reading another path or running another action until the result proves EOF. A truncated read is not evidence, cannot satisfy a readback gate, and makes promotion invalid if the Agent advances past it. Phase 0 final readbacks are stricter: do not use the file-read tool, `wc`, `sed`, `cat`, or another command. Run only the generated plan's `record-phase-0-readback.mjs <basename>` command. It emits one bounded numbered chunk and persists a hash-bound cursor; rerun the exact command it prints until EOF, then follow its next-file command. Promotion rejects missing, incomplete, out-of-order, or stale receipts.
...
- invoking `promote-phase-0.mjs` before completing the generated plan's entire ordered Phase 0 closeout sequence, including the artifact, gaps, and progress readbacks; a rejected early invocation is still a failed run and cannot be used to discover missing checklist items;
- passing design inputs, approved HTML, target paths, or any path outside `task-workflow/source/` as `prepare-phase-1-packet.mjs --evidence`, or reading the preparation script to reverse-engineer a rejected invocation instead of following its literal contract;
- guessing a target-owner path, or passing any `--files` value not copied exactly from `inventory-phase-1-owners.mjs` output `candidates[].path` and read separately first;
...
- after the Phase 1 first-packet gate passes, writing any target file before separately reading the first-packet receipt, updated Phase 1 artifact, and updated progress ledger in that exact order; gate stdout is never a receipt read;
...
| `scripts/record-phase-0-readback.mjs` | [Hash-bound ordered Phase 0 readback recorder](assets/scripts/record-phase-0-readback.mjs) |
| `scripts/inventory-phase-1-owners.mjs` | [Machine-generated existing target-owner inventory](assets/scripts/inventory-phase-1-owners.mjs) |
