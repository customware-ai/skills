...
Use code and the focused diff as implementation proof. Identify target owners and source evidence once, then build in dependency stages: first establish the shared design system in the existing Tailwind configuration and theme tokens; then adapt reusable UI components in `app/components/ui/` where the template uses that path; then build the app shell from those foundations; then implement the full routes/pages. Within each stage, do independent work concurrently wherever possible, with separate file ownership for shared code. Complete shared contracts before dependent work, but do not serialize independent components or pages. A page slice may span sections, state, and responsive rules when coupled. Inspect changed files, the diff, and connected callers/tests after a coherent implementation batch rather than after every small edit. Record only non-obvious adaptations, actual unresolved gaps, and the next action in the compact checkpoint; do not create per-slice ledgers or reread phase artifacts after each edit.

The target must have real route modules, source-ordered sections, source-backed copy/assets, usable local UI state and controls, responsive/theme adaptation, and no source HTML runtime dependency or invented backend/API/database/persistence/auth/business behavior. Implement intended source interactions that its documented runtime defect made unreachable, then prove them in the target phases. Disable unsupported navigation rather than inventing destinations. Use the real repository logo when available. Phase 1 is a time-bounded implementation pass: do not preemptively create browser packets or repeatedly check individual controls here. Target browser inspection, including exploratory inspection, runs only through the lifecycle helper; build/check is not browser proof.
...
Implement the source-represented breakpoints and adapt omitted sizes conservatively. Avoid overlap, clipping, horizontal canvas overflow, inaccessible controls, accidental document scroll, blank lower-sidebar space, or unreadable theme combinations. Do not add a theme switch unless required. Phase 1 completes implementation from source declarations, accepted source images, code review, and checks/build. Defer source-target screenshot comparison, browser measurements, and visual correction to Phase 2.
...
Review the final code/diff, check/build output, source-backed coverage, score arithmetic, critical items, and actual open gaps once. Record `Decision: Pass` and the evidence pointers in the Phase 1 artifact, set the marker to `phase-2-paired-responsive-proof`, update the checkpoint's phase/next action, and read the paired-fidelity and lifecycle references. Later visual corrections remain in Phase 2 or 3; refresh affected Phase 1 code/check evidence before promoting further, without repeating valid research or source capture.
