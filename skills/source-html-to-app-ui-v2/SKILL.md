...
| 2 | `phase-2-paired-responsive-proof` | broad paired source-target review of styling, placement, content, and responsive layout; immediate material visual repair | `48/50` | the overall UI looks source-faithful across routes, major states, sections, and viewports |
...
Complete source image review in Phase 0. In Phase 2, compare the broad UI route by route and area by area; Phase 3 completes the smaller-detail and interaction-state proof. Open each accepted/current image once at readable scale, individually or in small related batches; a concise pass/fail note for each compared area may cover several images when it names their paths and actual visual finding. Reuse already opened Phase 0 source findings and unchanged Phase 2 comparisons. Reopen only changed, stale, unclear, or unresolved pairs. Update the checkpoint only when the next action changes and gaps only for unresolved problems. Representative samples cannot replace required route, distinct visual-state, and readable-section coverage, but do not capture another image when current evidence already shows the same appearance. Compare content/order, geometry, spacing, typography, color, backgrounds, borders, radii, shadows, assets, controls, clipping, overlap, responsive transformation, interaction state, and scroll behavior in their owning phases.
