...
During Phase 0, create fresh task-workflow artifacts, capture and open the initial desktop/mobile source images, read the complete HTML/CSS/JavaScript, and inventory every page-like surface, state, section, interaction family, theme, breakpoint, scroll owner, and source-declared item not initially visible. Map each inventory item to planned desktop/mobile evidence before the complete capture; revise that map when discovery reveals a distinct state or unreadable area. Then capture the full source inventory and perform an image-by-image manifest walk: open every referenced image at a readable scale, with one concrete finding per image. The opened-image count must match the manifest count before Phase 0 passes; key-image samples or one-viewport review do not suffice. The exact tool order is flexible; the evidence and Phase 0 gate are not.
