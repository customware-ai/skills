...
```bash
pnpm exec node <customware-ai-skill>/scripts/install-ai-elements.mjs open-in-chat --cwd .
```

Underlying non-interactive shadcn command used by the installer:

```bash
pnpm dlx shadcn@latest add @ai-elements/open-in-chat --yes --path .tmp/customware-ai-elements-install
```
...
See [`open-in-chat.tsx`](examples/open-in-chat.tsx) for this official example.
