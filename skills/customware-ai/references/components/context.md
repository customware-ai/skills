...
```bash
pnpm exec node <customware-ai-skill>/scripts/install-ai-elements.mjs context --cwd .
```

Underlying non-interactive shadcn command used by the installer:

```bash
pnpm dlx shadcn@latest add @ai-elements/context --yes --path .tmp/customware-ai-elements-install
```
...
See [`context.tsx`](examples/context.tsx) for this official example.
