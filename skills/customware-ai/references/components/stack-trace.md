...
```bash
pnpm exec node <customware-ai-skill>/scripts/install-ai-elements.mjs stack-trace --cwd .
```

Underlying non-interactive shadcn command used by the installer:

```bash
pnpm dlx shadcn@latest add @ai-elements/stack-trace --yes --path .tmp/customware-ai-elements-install
```
...
See [`stack-trace.tsx`](examples/stack-trace.tsx) for this official example.
...
See [`stack-trace-collapsed.tsx`](examples/stack-trace-collapsed.tsx) for this official example.

### Hide Internal Frames

See [`stack-trace-no-internal.tsx`](examples/stack-trace-no-internal.tsx) for this official example.
