...
```bash
pnpm exec node <customware-ai-skill>/scripts/install-ai-elements.mjs prompt-input --cwd .
```

Underlying non-interactive shadcn command used by the installer:

```bash
pnpm dlx shadcn@latest add @ai-elements/prompt-input --yes --path .tmp/customware-ai-elements-install
```
...
See [`prompt-input.tsx`](examples/prompt-input.tsx) for this official example.
...
See [`prompt-input-cursor.tsx`](examples/prompt-input-cursor.tsx) for this official example.

### Button tooltips

Buttons can display tooltips with optional keyboard shortcut hints. Hover over the buttons below to see the tooltips.

See [`prompt-input-tooltip.tsx`](examples/prompt-input-tooltip.tsx) for this official example.
