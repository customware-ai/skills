...
```bash
pnpm exec node <customware-ai-skill>/scripts/install-ai-elements.mjs checkpoint --cwd .
```

Underlying non-interactive shadcn command used by the installer:

```bash
pnpm dlx shadcn@latest add @ai-elements/checkpoint --yes --path .tmp/customware-ai-elements-install
```
...
See [`checkpoint.tsx`](examples/checkpoint.tsx) for this official example.
