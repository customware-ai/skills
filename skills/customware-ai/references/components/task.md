...
```bash
pnpm exec node <customware-ai-skill>/scripts/install-ai-elements.mjs task --cwd .
```

Underlying non-interactive shadcn command used by the installer:

```bash
pnpm dlx shadcn@latest add @ai-elements/task --yes --path .tmp/customware-ai-elements-install
```
...
See [`task.tsx`](examples/task.tsx) for this official example.
