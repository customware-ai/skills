...
```bash
pnpm exec node <customware-ai-skill>/scripts/install-ai-elements.mjs reasoning --cwd .
```

Underlying non-interactive shadcn command used by the installer:

```bash
pnpm dlx shadcn@latest add @ai-elements/reasoning --yes --path .tmp/customware-ai-elements-install
```
...
See [`reasoning.tsx`](examples/reasoning.tsx) for this official example.
