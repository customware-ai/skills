...
```bash
pnpm exec node <customware-ai-skill>/scripts/install-ai-elements.mjs suggestion --cwd .
```

Underlying non-interactive shadcn command used by the installer:

```bash
pnpm dlx shadcn@latest add @ai-elements/suggestion --yes --path .tmp/customware-ai-elements-install
```
...
See [`suggestion.tsx`](examples/suggestion.tsx) for this official example.
...
See [`suggestion-input.tsx`](examples/suggestion-input.tsx) for this official example.
