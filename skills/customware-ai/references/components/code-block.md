...
```bash
pnpm exec node <customware-ai-skill>/scripts/install-ai-elements.mjs code-block --cwd .
```

Underlying non-interactive shadcn command used by the installer:

```bash
pnpm dlx shadcn@latest add @ai-elements/code-block --yes --path .tmp/customware-ai-elements-install
```
...
See [`code-block.tsx`](examples/code-block.tsx) for this official example.
...
See [`code-block-dark.tsx`](examples/code-block-dark.tsx) for this official example.
