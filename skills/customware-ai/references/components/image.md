...
```bash
pnpm exec node <customware-ai-skill>/scripts/install-ai-elements.mjs image --cwd .
```

Underlying non-interactive shadcn command used by the installer:

```bash
pnpm dlx shadcn@latest add @ai-elements/image --yes --path .tmp/customware-ai-elements-install
```
...
See [`image.tsx`](examples/image.tsx) for this official example.
