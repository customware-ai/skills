...
```bash
pnpm exec node <customware-ai-skill>/scripts/install-ai-elements.mjs inline-citation --cwd .
```

Underlying non-interactive shadcn command used by the installer:

```bash
pnpm dlx shadcn@latest add @ai-elements/inline-citation --yes --path .tmp/customware-ai-elements-install
```
...
See [`inline-citation.tsx`](examples/inline-citation.tsx) for this official example.
