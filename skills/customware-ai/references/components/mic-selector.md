...
```bash
pnpm exec node <customware-ai-skill>/scripts/install-ai-elements.mjs mic-selector --cwd .
```

Underlying non-interactive shadcn command used by the installer:

```bash
pnpm dlx shadcn@latest add @ai-elements/mic-selector --yes --path .tmp/customware-ai-elements-install
```
...
See [`mic-selector.tsx`](examples/mic-selector.tsx) for this official example.
