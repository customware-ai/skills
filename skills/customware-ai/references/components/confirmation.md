...
```bash
pnpm exec node <customware-ai-skill>/scripts/install-ai-elements.mjs confirmation --cwd .
```

Underlying non-interactive shadcn command used by the installer:

```bash
pnpm dlx shadcn@latest add @ai-elements/confirmation --yes --path .tmp/customware-ai-elements-install
```
...
See [`confirmation.tsx`](examples/confirmation.tsx) for this official example.
...
See [`confirmation-request.tsx`](examples/confirmation-request.tsx) for this official example.

### Approved State

Shows the accepted status when user approves and state is `approval-responded` or `output-available`.

See [`confirmation-accepted.tsx`](examples/confirmation-accepted.tsx) for this official example.

### Rejected State

Shows the rejected status when user rejects and state is `output-denied`.

See [`confirmation-rejected.tsx`](examples/confirmation-rejected.tsx) for this official example.
