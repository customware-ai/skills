...
```bash
pnpm exec node <customware-ai-skill>/scripts/install-ai-elements.mjs queue --cwd .
```

Underlying non-interactive shadcn command used by the installer:

```bash
pnpm dlx shadcn@latest add @ai-elements/queue --yes --path .tmp/customware-ai-elements-install
```
...
See [`queue.tsx`](examples/queue.tsx) for this official example.
...
See [`queue-prompt-input.tsx`](examples/queue-prompt-input.tsx) for this official example.
