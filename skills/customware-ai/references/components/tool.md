...
```bash
pnpm exec node <customware-ai-skill>/scripts/install-ai-elements.mjs tool --cwd .
```

Underlying non-interactive shadcn command used by the installer:

```bash
pnpm dlx shadcn@latest add @ai-elements/tool --yes --path .tmp/customware-ai-elements-install
```
...
See [`tool.tsx`](examples/tool.tsx) for this official example.
...
See [`tool-input-streaming.tsx`](examples/tool-input-streaming.tsx) for this official example.

### Input Available (Running)

Shows a tool that's actively executing with its parameters.

See [`tool-input-available.tsx`](examples/tool-input-available.tsx) for this official example.
...
See [`tool-output-available.tsx`](examples/tool-output-available.tsx) for this official example.

### Output Error

Shows a tool that encountered an error during execution. Opens by default to display the error.

See [`tool-output-error.tsx`](examples/tool-output-error.tsx) for this official example.
