...
> The copied example below demonstrates component APIs only. Keep `Message` and `MessageContent` inside the composed Tailwind chat surface described in [ai-elements-ui.md](../ai-elements-ui.md#chat-ui-quality-baseline); do not treat this isolated sample as the final product UI.

## Installation

```bash
pnpm exec node <customware-ai-skill>/scripts/install-ai-elements.mjs message --cwd .
```

Underlying non-interactive shadcn command used by the installer:

```bash
pnpm dlx shadcn@latest add @ai-elements/message --yes --path .tmp/customware-ai-elements-install
```
...
See [`message.tsx`](examples/message.tsx) for this official example.
