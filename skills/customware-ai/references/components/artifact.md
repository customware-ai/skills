...
```bash
pnpm exec node <customware-ai-skill>/scripts/install-ai-elements.mjs artifact --cwd .
```

Underlying non-interactive shadcn command used by the installer:

```bash
pnpm dlx shadcn@latest add @ai-elements/artifact --yes --path .tmp/customware-ai-elements-install
```
...
See [`artifact.tsx`](examples/artifact.tsx) for this official example.
