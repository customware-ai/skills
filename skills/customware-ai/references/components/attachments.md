...
```bash
pnpm exec node <customware-ai-skill>/scripts/install-ai-elements.mjs attachments --cwd .
```

Underlying non-interactive shadcn command used by the installer:

```bash
pnpm dlx shadcn@latest add @ai-elements/attachments --yes --path .tmp/customware-ai-elements-install
```
...
See [`attachments.tsx`](examples/attachments.tsx) for this official example.
...
See [`attachments-inline.tsx`](examples/attachments-inline.tsx) for this official example.

### List Variant

Best for file lists with full metadata display.

See [`attachments-list.tsx`](examples/attachments-list.tsx) for this official example.
