...
```bash
pnpm exec node <customware-ai-skill>/scripts/install-ai-elements.mjs speech-input --cwd .
```

Underlying non-interactive shadcn command used by the installer:

```bash
pnpm dlx shadcn@latest add @ai-elements/speech-input --yes --path .tmp/customware-ai-elements-install
```
...
See [`speech-input.tsx`](examples/speech-input.tsx) for this official example.
