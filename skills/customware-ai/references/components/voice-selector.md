...
```bash
pnpm exec node <customware-ai-skill>/scripts/install-ai-elements.mjs voice-selector --cwd .
```

Underlying non-interactive shadcn command used by the installer:

```bash
pnpm dlx shadcn@latest add @ai-elements/voice-selector --yes --path .tmp/customware-ai-elements-install
```
...
See [`voice-selector.tsx`](examples/voice-selector.tsx) for this official example.
...
```tsx
import { useVoiceSelector } from "~/components/ai-elements/voice-selector";
