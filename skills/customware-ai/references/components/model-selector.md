...
```bash
pnpm exec node <customware-ai-skill>/scripts/install-ai-elements.mjs model-selector --cwd .
```

Underlying non-interactive shadcn command used by the installer:

```bash
pnpm dlx shadcn@latest add @ai-elements/model-selector --yes --path .tmp/customware-ai-elements-install
```
...
See [`model-selector.tsx`](examples/model-selector.tsx) for this official example.
