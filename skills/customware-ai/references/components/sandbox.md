...
```bash
pnpm exec node <customware-ai-skill>/scripts/install-ai-elements.mjs sandbox --cwd .
```

Underlying non-interactive shadcn command used by the installer:

```bash
pnpm dlx shadcn@latest add @ai-elements/sandbox --yes --path .tmp/customware-ai-elements-install
```
...
See [`sandbox.tsx`](examples/sandbox.tsx) for this official example.
