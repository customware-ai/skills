...
```bash
pnpm exec node <customware-ai-skill>/scripts/install-ai-elements.mjs agent --cwd .
```

Underlying non-interactive shadcn command used by the installer:

```bash
pnpm dlx shadcn@latest add @ai-elements/agent --yes --path .tmp/customware-ai-elements-install
```
...
See [`agent.tsx`](examples/agent.tsx) for this official example.
