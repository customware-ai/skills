...
```bash
pnpm exec node <customware-ai-skill>/scripts/install-ai-elements.mjs web-preview --cwd .
```

Underlying non-interactive shadcn command used by the installer:

```bash
pnpm dlx shadcn@latest add @ai-elements/web-preview --yes --path .tmp/customware-ai-elements-install
```
...
See [`web-preview.tsx`](examples/web-preview.tsx) for this official example.
