...
```bash
pnpm exec node <customware-ai-skill>/scripts/install-ai-elements.mjs commit --cwd .
```

Underlying non-interactive shadcn command used by the installer:

```bash
pnpm dlx shadcn@latest add @ai-elements/commit --yes --path .tmp/customware-ai-elements-install
```
...
See [`commit.tsx`](examples/commit.tsx) for this official example.
