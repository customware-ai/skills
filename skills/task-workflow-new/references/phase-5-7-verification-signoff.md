...
Use this contract shape for Phase 5, Phase 6, and Phase 7:

| Phase | Goal | Allowed work | Evidence required | Stop condition | Fallback condition |
| --- | --- | --- | --- | --- | --- |
| Phase 5 | Prove changed behavior and responsive UI in a real browser | Standalone Playwright scripts through lifecycle helper, screenshots, bad-case and surrounding-flow checks | Helper command, readiness, logs, scripts, screenshots, viewport evidence, issues/fixes | Phase 5 gate passes with no unresolved UI/runtime/responsive issue | Manual/server fallback only after diagnosed helper failure and recorded logs |
| Phase 6 | Protect changed behavior with durable targeted/connected coverage | Existing E2E review, updated/new targeted specs, helper-owned E2E runs, connected lower-level tests when better fit | Test selection ledger, exact command/log output, lifecycle/setup reuse, fixed-wait review | Phase 6 gate passes with targeted/connected evidence current | Full E2E only when task explicitly asks or exact-task repo instruction requires it |
| Phase 7 | Audit every artifact and complete the MITB task only after gates pass | Artifact re-open, final diff review, evidence consistency check, completion command | Final artifact integrity review, scorecard, completion command output | Phase 7 gate passes and task completion command succeeds | Return to earliest failing phase if any audit item fails |

Do not add verification just to feel safer. Add or rerun verification only when the phase contract says evidence can change: code/test/config/setup changed, prior output is stale/incomplete, or a narrower diagnostic is needed to repair a real failure.
...
- Its Compact State Ledger must state whether Phase 3 build evidence, browser lifecycle state, DB/setup state, and E2E outputs are reusable or invalidated.
- Its Phase Contract Ledger must point to the evidence that lets a reviewer decide whether reruns are needed.
- Its Invalidation Ledger must state whether build, browser lifecycle, DB/setup, fixture, test, or E2E evidence stayed valid after the last related change.
...
For any Phase 5 or Phase 6 fallback away from the default lifecycle helper, record the full fallback chain in the owning artifact and `progress.md`:

- helper/default command attempted
- helper/default log path and failure or mismatch
- reason fallback is allowed for this exact command
- fallback owner used, such as repo `webServer`, explicit PID/port cleanup, or manual PID-managed server
- cleanup result and whether the resulting state can be reused

If this evidence is missing, the phase cannot pass. Return to the phase and either use the default helper or record the fallback correctly.
...
Phase 7 audits evidence. It should not start new checks, tests, builds, browser probes, or E2E runs just to feel safe.

If Phase 7 finds missing, stale, contradicted, or invalidated evidence, return to the owning earlier phase:

- static/build evidence issue -> Phase 3
- integrity/check/test-selection issue -> Phase 4
- browser/responsive/lifecycle issue -> Phase 5
- E2E/setup/DB/test-output issue -> Phase 6
- artifact consistency issue -> the earliest artifact that is wrong

Only after the owning phase repairs and re-passes may Phase 7 audit again.
