...
| --- | --- | --- |
| Goal | Audit every artifact and complete the MITB task only after gates pass | Pending |
| Allowed work used | Artifact re-open, final diff review, evidence consistency check, completion command | Pending |
| Evidence required | Final artifact integrity review, scorecard, and completion command output | Pending |
| Stop condition | Phase 7 gate passes and task completion command succeeds | Pending |
| Fallback condition | Any failed audit item returns to the earliest failing phase before completion | Pending |
...
| Fallbacks, if any, recorded default attempt, failure evidence, allowed reason, fallback owner/command, and cleanup/reuse result | Pending |
| Invalidation ledger reviewed; stale or invalidated evidence routed back to owning phase | Pending |
| Reviewer-facing evidence is compact and enough to decide whether reruns are needed | Pending |
| Phase 7 did not start fresh checks/tests/builds/browser/E2E just for confidence; stale/missing evidence returned to owning phase | Pending |
...
| Fallbacks, if any, have default-attempt, failure, allowed-reason, fallback-owner, and cleanup/reuse evidence | yes | Pending |
| Invalidation ledger is current and any stale evidence was routed back to its owning phase | yes | Pending |
| Reviewer-facing evidence is compact and decision-ready | yes | Pending |
| Phase 7 did not run fresh checks/tests/builds/browser/E2E only for confidence | yes | Pending |
