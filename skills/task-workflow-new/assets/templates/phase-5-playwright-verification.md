...
| --- | --- | --- |
| Goal | Prove changed behavior and responsive UI in a real browser | Pending |
| Allowed work used | Standalone Playwright scripts through lifecycle helper, screenshots, bad-case and surrounding-flow checks | Pending |
| Evidence required | Helper command, readiness, logs, scripts, screenshots, viewport evidence, issues/fixes | Pending |
| Stop condition | Phase 5 gate passes with no unresolved UI/runtime/responsive issue | Pending |
| Fallback condition | Manual/server fallback used only after diagnosed helper failure with recorded logs | Pending |
...
Fill this only if Phase 5 used repo lifecycle or manual fallback instead of the lifecycle helper default.
