...
| --- | --- | --- |
| Goal | Protect changed behavior with durable targeted/connected coverage | Pending |
| Allowed work used | Existing E2E review, updated/new targeted specs, helper-owned E2E runs, connected lower-level tests when better fit | Pending |
| Evidence required | Test selection ledger, exact command/log output, lifecycle/setup reuse, and fixed-wait review | Pending |
| Stop condition | Phase 6 gate passes with targeted/connected evidence current | Pending |
| Fallback condition | Full E2E only when task explicitly asks or exact-task repo instruction requires it | Pending |
...
Fill this only if Phase 6 used native Playwright `webServer`, manual server ownership, explicit PID/port cleanup, or another fallback instead of the lifecycle helper default.

| Default/helper attempt | Failure or mismatch evidence | Why fallback was allowed | Fallback owner/command | Cleanup/reuse result |
| --- | --- | --- | --- | --- |
| None used | N/A | N/A | N/A | N/A |
