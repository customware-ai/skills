...
| --- | --- | --- |
| Goal | Close connected implementation gaps and produce reusable static/build evidence | Pending |
| Allowed work used | Connected-place inspection, repairs, then ordered typecheck, lint, build | Pending |
| Evidence required | Issue groups, batch fixes, final typecheck/lint/build result, and reusable build evidence | Pending |
| Stop condition | Phase 3 gate passes and build evidence is reusable by later phases | Pending |
| Fallback condition | Combined static/build command used only when it is the repo's actual contract | Pending |
